...
void        rtwn_rx_frame(struct rtwn_pci_softc *,
            ...
            struct mbuf_list *);
int        rtwn_tx(void *, mbuf_t, struct ieee80211_node *);
...
{
    return (pci_matchbyid((struct pci_attach_args *)aux, rtwn_pci_devices,
    ...
    struct rtwn_rx_data *rx_data, int desc_idx, struct mbuf_list *ml)
{
    ...
    m = rx_data->m;
    rx_data->m = m1;
    mbuf_pkthdr_setlen(m, pktlen + infosz + shift);
    mbuf_setlen(m, pktlen + infosz + shift);
...
rtwn_tx(void *cookie, mbuf_t m, struct ieee80211_node *ni)
{
    ...
    txd->txdw0 = htole32(
        SM(R92C_TXDW0_PKTLEN, mbuf_pkthdr_len(m)) |
    ...
    if (!IEEE80211_IS_MULTICAST(wh->i_addr1) &&
        ...
        if (mbuf_pkthdr_len(m) + IEEE80211_CRC_LEN > ic->ic_rtsthreshold) {
    ...
    txd->txbufsize = htole16(mbuf_pkthdr_len(m));
    bus_space_barrier(sc->sc_st, sc->sc_sh, 0, sc->sc_mapsize,
        BUS_SPACE_BARRIER_WRITE);
