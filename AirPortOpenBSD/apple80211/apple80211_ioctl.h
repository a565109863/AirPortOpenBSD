...
{
    u_int32_t                    version;                           // 0x0 0x1 0x2 0x3
    u_int32_t                    bss_type;                          // apple80211_apmode 0x4 0x5 0x6 0x7
    struct ether_addr            bssid;                             // target BSSID 0x8 0x9 0xa 0xb 0xc 0xd
    u_int16_t                    unk;                               // unk 0xe 0xf
    u_int32_t                    ssid_len;                          // length of the SSID 0x10 0x11 0x12 0x13
    u_int8_t                     ssid[APPLE80211_MAX_SSID_LEN];     // direct scan ssid or AirDrop scan ssid like "Air-xxxx"  0x14 ~  0x33
    u_int32_t                    scan_type;                         // apple80211_scan_type 0x34 0x35 0x36 0x37
    u_int32_t                    phy_mode;                          // apple80211_phymode vector 0x38 0x39 0x3a 0x3b
    u_int16_t                    dwell_time;                        // time to spend on each channel (ms) 0x3c 0x3d
    u_int16_t                    unk_time;                          // time between scanning each channel (ms) 0x3e 0x3f
    u_int32_t                    rest_time;                         // time between scanning each channel (ms) 0x40 0x41 0x42 0x43
    u_int32_t                    num_channels;                      // 0 if not passing in channels 0x44 0x45 0x46 0x47
    struct apple80211_channel    channels[APPLE80211_MAX_CHANNELS]; // channel list  0x48 0x49 0x4a 0x4b 0x644 0x645 0x646 0x647
    u_int8_t                     scan_flags;                        // 0x648
};

struct apple80211_scan_multiple_data
{
    uint32_t                  version;      // 0x0 0x1 0x2 0x3
    uint32_t                  ap_mode;      // apple80211_apmode  0x4 0x5 0x6 0x7
    uint32_t                  ssid_count;   // 0x8 0x9 0xa 0xb
    apple80211_ssid_data      ssids[10];    // 0xc 0x19b
    uint32_t                  bssid_count;  // 0x19c 0x19d 0x19e 0x19f
    ether_addr                bssids[16];   // 0x1A0 0x1FF 6*16=96
    uint32_t                  scan_type;    // 0x200 0x201 0x202 0x203
    uint32_t                  phy_mode;     // 0x204 0x205 0x206 0x207
    uint32_t                  dwell_time;   // 0x208 0x209 0x20a 0x20b
    uint32_t                  rest_time;    // 0x20c 0x20d 0x20e 0x20f
    uint32_t                  num_channels; // 0x210 0x211 0x212 0x213
    struct apple80211_channel channels[APPLE80211_MAX_CHANNELS]; // 0x214 ~  0x813
    uint16_t                  unk_2;                             // 0x814 0x815
};
